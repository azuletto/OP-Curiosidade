...
submitButton.addEventListener("click", function (e) {
      ...
      if (verfifyUser(user_edit)) {
        ...
        users_list.find((user) => String(user.id) === String(user_edit.id)).age =
          user_edit.age;
        users_list.find((user) => String(user.id) === String(user_edit.id)).email =
          user_edit.email;
        users_list.find((user) => String(user.id) === String(user_edit.id)).adress =
          user_edit.adress;
        users_list.find((user) => String(user.id) === String(user_edit.id)).info =
          user_edit.info;
        users_list.find((user) => String(user.id) === String(user_edit.id)).interess =
          user_edit.interess;
        users_list.find((user) => String(user.id) === String(user_edit.id)).feelings =
          user_edit.feelings;
        users_list.find((user) => String(user.id) === String(user_edit.id)).valors =
          user_edit.valors;
        users_list.find((user) => String(user.id) === String(user_edit.id)).status =
          user_edit.status;
